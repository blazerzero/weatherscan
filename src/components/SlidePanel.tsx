  ...
  return (
    <section className="flex flex-col h-full">
      ...
      <div
        ...
        style={{ borderTop: '2px solid #1a4070' }}
      >
        {slides.map((slide, i) => {
          ...
          return (
            <button
              ...
              style={{
                ...
                color: active
                  ? (isAlert ? '#ff8800' : '#00aaff')
                  : '#557799',
                borderRight: i < slides.length - 1 ? '1px solid #1a4070' : undefined,
                textShadow: active ? '0 0 8px currentColor' : undefined,
                border: 'none',
                outline: 'none',
              }}
            >
              {isAlert && data.alerts.length > 0 ? `⚠ ${SLIDE_LABELS[slide]}` : SLIDE_LABELS[slide]}
            </button>
          )
        })}
      </div>
