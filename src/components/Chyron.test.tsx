...
describe('Chyron', () => {
  it('renders the location badge when there are no alerts', () => {
    render(<Chyron localAlerts={[]} locationName="Philadelphia" />, { wrapper })
    expect(screen.getByText(/PHILAD/)).toBeInTheDocument() // truncated to 8 chars
  })

  it('renders the ALERT badge when alerts are present', () => {
    render(<Chyron localAlerts={[ALERT]} locationName="Philadelphia" />, { wrapper })
    expect(screen.getByText(/ALERT/)).toBeInTheDocument()
  })

  it('renders the scrolling track element', () => {
    const { container } = render(<Chyron localAlerts={[]} locationName="Philadelphia" />, { wrapper })
