...
describe('kmToMiles', () => {
  ...
  it('converts 16.09 km (~10 miles)', () => expect(kmToMiles(16.09)).toBeCloseTo(10, 0))
...
describe('mpsToMph', () => {
  ...
  it('converts 44.7 m/s (~100 mph)', () => expect(mpsToMph(44.7)).toBeCloseTo(100, 0))
})
...
describe('wmoLabel', () => {
  describe('clear / cloudy', () => {
    ...
    it('code 1 → Mostly Clear', () => expect(wmoLabel(1, true)).toBe('Mostly Clear'))
    it('code 2 → Partly Cloudy', () => expect(wmoLabel(2, true)).toBe('Partly Cloudy'))
  ...
  describe('drizzle', () => {
    it('code 51 → Light Drizzle', () => expect(wmoLabel(51, true)).toBe('Light Drizzle'))
    it('code 53 → Drizzle', () => expect(wmoLabel(53, true)).toBe('Drizzle'))
    it('code 55 → Heavy Drizzle', () => expect(wmoLabel(55, true)).toBe('Heavy Drizzle'))
    it('code 56 → Freezing Drizzle', () => expect(wmoLabel(56, true)).toBe('Freezing Drizzle'))
    it('code 57 → Freezing Drizzle', () => expect(wmoLabel(57, true)).toBe('Freezing Drizzle'))
  })

  describe('rain', () => {
    it('code 61 → Light Rain', () => expect(wmoLabel(61, true)).toBe('Light Rain'))
    it('code 63 → Rain', () => expect(wmoLabel(63, true)).toBe('Rain'))
    it('code 65 → Heavy Rain', () => expect(wmoLabel(65, true)).toBe('Heavy Rain'))
    it('code 66 → Freezing Rain', () => expect(wmoLabel(66, true)).toBe('Freezing Rain'))
    it('code 67 → Freezing Rain', () => expect(wmoLabel(67, true)).toBe('Freezing Rain'))
  })

  describe('snow', () => {
    it('code 71 → Light Snow', () => expect(wmoLabel(71, true)).toBe('Light Snow'))
    it('code 73 → Snow', () => expect(wmoLabel(73, true)).toBe('Snow'))
    it('code 75 → Heavy Snow', () => expect(wmoLabel(75, true)).toBe('Heavy Snow'))
    it('code 77 → Snow Grains', () => expect(wmoLabel(77, true)).toBe('Snow Grains'))
  })

  describe('showers', () => {
    it('code 80 → Light Showers', () => expect(wmoLabel(80, true)).toBe('Light Showers'))
    it('code 81 → Showers', () => expect(wmoLabel(81, true)).toBe('Showers'))
    it('code 82 → Heavy Showers', () => expect(wmoLabel(82, true)).toBe('Heavy Showers'))
    it('code 85 → Snow Showers', () => expect(wmoLabel(85, true)).toBe('Snow Showers'))
    it('code 86 → Snow Showers', () => expect(wmoLabel(86, true)).toBe('Snow Showers'))
  })

  describe('thunderstorms', () => {
    it('code 95 → Thunderstorm', () => expect(wmoLabel(95, true)).toBe('Thunderstorm'))
    it('code 96 → Severe Thunderstorm', () => expect(wmoLabel(96, true)).toBe('Severe Thunderstorm'))
    it('code 99 → Severe Thunderstorm', () => expect(wmoLabel(99, true)).toBe('Severe Thunderstorm'))
  })

  it('unknown code → Unknown', () => expect(wmoLabel(999, true)).toBe('Unknown'))
})
...
const MOCK_OPEN_METEO_RESPONSE = {
  ...
  daily: {
    ...
    sunrise: ['2024-01-15T07:00', '2024-01-16T07:01', '2024-01-17T07:01', '2024-01-18T07:02', '2024-01-19T07:02', '2024-01-20T07:02', '2024-01-21T07:03'],
    sunset: ['2024-01-15T17:30', '2024-01-16T17:31', '2024-01-17T17:32', '2024-01-18T17:33', '2024-01-19T17:34', '2024-01-20T17:35', '2024-01-21T17:36'],
  },
}
...
describe('fetchCurrentAndForecast', () => {
  ...
  it('converts current temperature from Celsius to Fahrenheit', async () => {
    const { current } = await fetchCurrentAndForecast({ lat: 40.7, lon: -74.0 })
  ...
  it('throws on non-ok response', async () => {
    ...
    await expect(fetchCurrentAndForecast({ lat: 40.7, lon: -74.0 })).rejects.toThrow('Open-Meteo error: 500')
  })
...
describe('reverseGeocode', () => {
  ...
  it('falls back to town if city is missing', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ address: { town: 'Smallville', country_code: 'us' } }),
    }))
  ...
  it('marks non-US locations correctly', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ address: { city: 'London', state: 'England', country_code: 'gb' } }),
    }))
