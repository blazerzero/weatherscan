import type { Coordinates, CurrentConditions, HourlyPoint, DailyForecast, LocationInfo } from '../types/weather'
...
export async function fetchCurrentAndForecast(coords: Coordinates): Promise<{
  ...
  const hourly: HourlyPoint[] = data.hourly.time.slice(0, 12).map((t, i) => ({
    ...
    conditionLabel: wmoLabel(data.hourly.weather_code[i] ?? 0, (data.hourly.is_day[i] ?? 1) === 1),
...
export async function reverseGeocode(coords: Coordinates): Promise<LocationInfo | null> {
