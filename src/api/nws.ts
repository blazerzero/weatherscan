...
const HEADERS = { 'User-Agent': 'WeatherScan/1.0 (weather@weatherscan.app)', Accept: 'application/ld+json' }

/** Enrich a US LocationInfo with NWS grid data (office + zone ID) */
export async function enrichWithNWS(location: LocationInfo): Promise<LocationInfo> {
  ...
  try {
    ...
    const [, office, gridX, gridY] =
      (data.forecastGridData ?? '').match(/\/gridpoints\/([A-Z]+)\/(\d+),(\d+)/) ?? []
    return {
      ...location,
      nwsOffice: office,
...
export async function fetchAlerts(location: LocationInfo): Promise<WeatherAlert[]> {
  ...
  try {
    ...
    const res = await fetch(`${BASE}/alerts/active?${params}`, { headers: HEADERS })
    if (!res.ok) return []
...
export async function fetchNationalHeadlines(): Promise<string[]> {
  try {
    ...
    if (!res.ok) return []
    const data = await res.json() as NWSAlertsResponse
    return (data['@graph'] ?? []).map((a) => `${a.event}: ${a.areaDesc ?? ''}`.trim())
  } catch {
    return []
  }
}
