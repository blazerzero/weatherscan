...
describe('radarTileUrl', () => {
  it('builds a valid Leaflet tile URL template', () => {
    const url = radarTileUrl('https://tilecache.rainviewer.com', '/v2/radar/1234567890', 2, 1, 0)
    expect(url).toBe('https://tilecache.rainviewer.com/v2/radar/1234567890/256/{z}/{x}/{y}/2/1_0.png')
  })

  it('uses default color scheme (2), smooth (1), snow (0) when not specified', () => {
    const url = radarTileUrl('https://tilecache.rainviewer.com', '/v2/radar/abc')
    expect(url).toContain('/2/1_0.png')
  })

  it('respects custom color scheme', () => {
    const url = radarTileUrl('https://tilecache.rainviewer.com', '/v2/radar/abc', 4)
    expect(url).toContain('/4/')
  })

  it('respects smooth=0', () => {
    const url = radarTileUrl('https://tilecache.rainviewer.com', '/v2/radar/abc', 2, 0)
    expect(url).toContain('0_0.png')
  })

  it('respects snow=1', () => {
    const url = radarTileUrl('https://tilecache.rainviewer.com', '/v2/radar/abc', 2, 1, 1)
